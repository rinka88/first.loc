...
<head>
    ...
    */
    ...
    $statment->execute();
    $peoples = $statment ->fetchAll(PDO::FETCH_ASSOC);
    ?>
</head>
                    ...
                    <div class="d-flex flex-wrap demo demo-h-spacing mt-3 mb-3">
                        <?php foreach ($peoples as $people): ?>
                            <div class="<?php echo $people['status'] == 'banned' ? 'banned' : '' ?> rounded-pill bg-white shadow-sm p-2 border-faded mr-3 d-flex flex-row align-items-center justify-content-center flex-shrink-0">
                                <img src="<?php echo $people['image'];?>" alt="<?php echo $people['image_ait'];?>" class="img-thumbnail img-responsive rounded-circle" style="width:5rem; height: 5rem;">
                                <div class="ml-2 mr-3">
                                    <h5 class="m-0">
                                        <?php echo $people['name'];?>
                                        <small class="m-0 fw-300">
                                            <?php echo $people['job_title'];?>
                                        </small>
                                    </h5>
                                    <a href="<?php echo $people['twitter_link_href'];?>" class="text-info fs-sm" target="_blank"><?php echo $people['twitter_text'];?></a> -
                                    <a href="<?php echo $people['email_link_href'];?>" class="text-info fs-sm" target="_blank" title="<?php echo $people['email_link_title'];?>"><?php echo $people['email_link_text'];?></a>
                                </div>
                            </div>
                        <?php endforeach;?>
                    </div>
